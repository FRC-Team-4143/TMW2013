...
if (setpoint < 0)
{
	return setpoint + 1024;
}
else if (setpoint > 1024)
{
	return setpoint - 1024;
}
else if (setpoint == 1024)
...
{	
	if(driveFront) {
		...
		if(fabs((int)FLSetPoint + FLOffset - frontLeftPos->GetAverageValue()) < 256 || fabs((int)FLSetPoint + FLOffset - frontLeftPos->GetAverageValue()) > 768)
		{
		...
		{
			frontLeft->SetSetpoint(CorrectSteerSetpoint((int)FLSetPoint + FLOffset-512));
			FLInv = -1;
		}
		
		if(fabs((int)FRSetPoint + FROffset - frontRightPos->GetAverageValue()) < 256 || fabs((int)FRSetPoint + FROffset - frontRightPos->GetAverageValue()) > 768)
		{
		...
		{
			frontRight->SetSetpoint(CorrectSteerSetpoint((int)FRSetPoint + FROffset-512));
			FRInv = -1;
		}
		
		if(fabs((int)RLSetPoint + RLOffset - rearLeftPos->GetAverageValue()) < 256 || fabs((int)RLSetPoint + RLOffset - rearLeftPos->GetAverageValue()) > 768)
		{
		...
		{
			rearLeft->SetSetpoint(CorrectSteerSetpoint((int)RLSetPoint + RLOffset-512));
			RLInv = -1;
		}
		
		if(fabs((int)RRSetPoint + RROffset - rearRightPos->GetAverageValue()) < 256 || fabs((int)RRSetPoint + RROffset - rearRightPos->GetAverageValue()) > 768)
		{
		...
		{
			rearRight->SetSetpoint(CorrectSteerSetpoint((int)RRSetPoint + RROffset-512));
			RRInv = -1;
		}
...
void DriveTrain::SetDriveSpeed(float FLSpeed, float FRSpeed, float RLSpeed, float RRSpeed) {
	...
	if(driveFront) {
		...
		rearLeftDrive->Set(FRSpeed*FRInv);
...
void DriveTrain::Pivot(float twistinput, float speedinput)
{
	...
	SetSteerSetpoint(floor(512/pi*thetaFL),floor(512/pi*thetaFR),floor(512/pi*thetaRL),floor(512/pi*thetaRR));
}
