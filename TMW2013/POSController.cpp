 ...
 */
POSController::POSController(float Kp, float Ki, float Kd,
 ...
 * integral and differental terms. The default is 50ms.
 */
POSController::POSController(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period) :
	m_semaphore (0)
{
...
void POSController::Initialize(float Kp, float Ki, float Kd, float Kf,
								PIDSource *source, PIDOutput *output,
								float period)
{
	m_table = NULL;
	...
	m_controlLoop = new Notifier(POSController::CallCalculate, this);
 ...
 *g */
void POSController::CallCalculate(void *controller)
{
	POSController *control = (POSController*) controller;
	control->Calculate();
}
    ...
    bool ontarget;

		{
  		...
  		case kPercentTolerance:
  			ontarget = fabs(GetError()) < (m_tolerance / 100 * (m_maximumInput - m_minimumInput));
  			break;
  		case kAbsoluteTolerance:
  			ontarget = fabs(GetError()) < m_tolerance;
  			break;
      default:
        ontarget = false;
		  }
 ...
 */
void POSController::SetPID(float p, float i, float d)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void POSController::SetPID(float p, float i, float d, float f)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_P = p;
 ...
 */
void POSController::SetContinuous(bool continuous)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void POSController::SetInputRange(float minimumInput, float maximumInput)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void POSController::SetOutputRange(float minimumOutput, float maximumOutput)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Set the setpoint for the POSController
 * @param setpoint the desired setpoint
 */
void POSController::SetSetpoint(float setpoint)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Returns the current setpoint of the POSController
 * @return the current setpoint
 */
float POSController::GetSetpoint()
{
	float setpoint;
	CRITICAL_REGION(m_semaphore)
	{
 ...
 */
void POSController::SetTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
 ...
 * @param percentage error which is tolerable
 */
void POSController::SetPercentTolerance(float percent)
{
	CRITICAL_REGION(m_semaphore)
	{
		m_toleranceType = kPercentTolerance;
 ...
 * @param percentage error which is tolerable
 */
void POSController::SetAbsoluteTolerance(float absTolerance)
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Begin running the POSController
 */
void POSController::Enable()
{
	CRITICAL_REGION(m_semaphore)
	{			
		m_enabled = true;
...
/**
 * Stop running the POSController, this sets the output to zero before stopping.
 */
void POSController::Disable()
{
	CRITICAL_REGION(m_semaphore)
	{
...
/**
 * Return true if POSController is enabled.
 */
bool POSController::IsEnabled()
{
	bool enabled;
	CRITICAL_REGION(m_semaphore)
	{
...
std::string POSController::GetSmartDashboardType(){
	return "POSController";
}

void POSController::InitTable(ITable* table){
	if(m_table!=NULL)
...
ITable* POSController::GetTable(){
	return m_table;
}

void POSController::ValueChanged(ITable* source, const std::string& key, EntryValue value, bool isNew){
...
void POSController::UpdateTable() {
	
}

void POSController::StartLiveWindowMode() {
	Disable();
}

void POSController::StopLiveWindowMode() {
