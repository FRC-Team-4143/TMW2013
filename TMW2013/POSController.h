...
#ifndef POSCONTROLLER_H_
#define POSCONTROLLER_H_
 ...
 * PIDOutput
 */
class POSController : public LiveWindowSendable, public Controller, public ITableListener
{
public:
	POSController(float p, float i, float d,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	POSController(float p, float i, float d, float f,
					PIDSource *source, PIDOutput *output,
					float period = 0.05);
	virtual ~POSController();
	virtual float Get();
...
protected:
	...
	DISALLOW_COPY_AND_ASSIGN(POSController);
};
